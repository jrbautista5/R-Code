...
#b1 <- mgcv::gam(Approve ~ s(PriorDefault, bs="ps", sp=lam) + s(YearsEmployed, bs='ps', sp=lam) + 
#                 s(CreditScore, bs='ps', sp=lam) + s(Income, bs='ps', sp=lam), data = train, method="REML")
#summary(b1)
#plot(b1)
...
# library(gam)
b4 <- gam::gam(Approve ~ lo(YearsEmployed, span=0.6) + 
           lo(CreditScore, span=0.6) + lo(Income, span=0.6), data = train)
summary(b4)
#plot(b4)
prediction <- predict(b4, newdata=test)
